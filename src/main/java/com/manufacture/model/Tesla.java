...
public class Tesla extends Functionaries implements NewCarModel {

    private List<Ability> abilities = new ArrayList<>();
    private Car car;

    public Tesla(String color, String fuelType, int numberOfGear) {
        try {
            this.abilities.add(runOnBattery());

            if (Objects.nonNull(color) && Objects.nonNull(fuelType) && Utils.checkNumberOfGear(numberOfGear)) {
                this.car = new Car(ModelName.TESLA, color, fuelType, numberOfGear);
            }
