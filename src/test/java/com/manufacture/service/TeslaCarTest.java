...
public class TeslaCarTest {
    ...
    public void test_manufactureCar_tesla() {
        ...
        doNothing().when(mockCar).setCarFeatures(CarFeatures.ABILITY_TO_RUN_ON_BATTERY);
        teslaCar.manufactureCar(mockCar);
        verify(mockCar, VerificationModeFactory.times(1)).setModelName(ModelName.TESLA);
        verify(mockCar, VerificationModeFactory.times(1)).setCarFeatures(CarFeatures.ABILITY_TO_RUN_ON_BATTERY);
    }
}